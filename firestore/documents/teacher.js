...
export class Teachers {
  ...
  static deleteTeacher = async (teacherID) => {
    try {
      await deleteDoc(doc(db, Teachers.collectionName.toString(), teacherID));
  ...
  static getTeachersCreatedByUser = async (userID) => {
    try {
      const data = [];
      const queryRes = query(collection(db, Teachers.collectionName.toString()), where("createdBy", "==", userID));
  ...
  static getTeachersBySchool = async (schoolID) => {
    try {
      const data = [];
      const queryRes = query(collection(db, Teachers.collectionName.toString()), where("schoolID", "==", schoolID));
      const querySnapshot = await getDocs(queryRes);
      querySnapshot.forEach((doc) => {
        data.push({ teacherID: doc.id, ...doc.data() });
      });
